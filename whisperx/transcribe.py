...
DEFAULT_AUDIO_DIR = "/teamspace/studios/this_studio/audio"
SUPPORTED_AUDIO_EXTENSIONS = ('.mp3', '.wav', '.ogg', '.flac', '.m4a', '.aac')

def cli():
    # fmt: off
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("--audio", nargs="*", type=str, help="specific audio file(s) to transcribe (overrides audio_dir)")
    parser.add_argument("--audio_dir", type=str, default=DEFAULT_AUDIO_DIR, help="directory containing audio files to transcribe")
    ...
    specific_files = args.pop("audio")
    audio_dir = args.pop("audio_dir")
    
    if specific_files:
        audio_files = specific_files
    else:
    ...
    for audio_path in audio_files:
        ...
        # >> VAD & ASR
        if verbose:
            print(f">> Performing transcription on {os.path.basename(audio_path)}...")
    ...
    if not no_align:
        ...
        for result, audio_path in tmp_results:
            ...
            if align_model is not None and len(result["segments"]) > 0:
                ...
                if verbose:
                    print(f">> Performing alignment on {os.path.basename(audio_path)}...")
